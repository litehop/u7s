...
if [ -f "$WORKDIR/ca.crt" ]; then
  ...
  # konnectivity-server's --server-ca-cert trust bundle must ALSO include the dedicated
  # front-proxy CA: build_backend_client (aggregation.rs) presents a cert signed by THAT
  # CA -- not the cluster CA -- when tunneling to an aggregated backend through
  # konnectivity, so konnectivity-server must accept it too, or every aggregated-backend
  # proxy call fails the outer CONNECT handshake before ever reaching the backend. Both
  # CAs are u7s's own, generated by the same apiserver process -- bundling them widens
  # konnectivity's client-auth trust anchor, not its actual access boundary (only u7s
  # itself ever holds either private key).
  if [ -f "$WORKDIR/proxy-client-ca.crt" ]; then
    openssl x509 -inform DER -in "$WORKDIR/proxy-client-ca.crt" >> "$WORKDIR/ca.pem"
  fi
...
if [ "$BACKGROUND" -eq 1 ]; then
  ...
  "$BINARY" \
    ...
    --proxy-client-ca-key  "$WORKDIR/proxy-client-ca.key" \
    --proxy-client-ca-cert "$WORKDIR/proxy-client-ca.crt" \
...
if [ -f "$WORKDIR/ca.crt" ] && ! nc -z "$HOST_IP" $KONNECTIVITY_PROXY_PORT 2>/dev/null; then
  ...
  openssl x509 -inform DER -in "$WORKDIR/ca.crt" -out "$WORKDIR/ca.pem"
  # See the matching block above: bundle the dedicated front-proxy CA into
  # konnectivity-server's client-auth trust anchor too.
  if [ -f "$WORKDIR/proxy-client-ca.crt" ]; then
    openssl x509 -inform DER -in "$WORKDIR/proxy-client-ca.crt" >> "$WORKDIR/ca.pem"
  fi
