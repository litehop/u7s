...
write_manifest_copy_runner() {
  local install_script="$1" runner="$2"
  {
    ...
    echo 'JOIN_MODE="$3"'
    awk '/^if \[ "\$JOIN_MODE" -eq 0 \]; then$/,/^fi$/' "$install_script"
  } > "$runner"
}
...
# Case 3: JOIN_MODE=1 (a --join worker install, no apiserver to apply
...
assert_false "--join (JOIN_MODE=1) skips the manifest copy step entirely -- a worker node runs no apiserver, so there is nothing to auto-apply" \
  test -e "$dest3"
