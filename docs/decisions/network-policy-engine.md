...
- If a future revisit compares this engine against alternatives, the `[sig-network] Netpol` upstream conformance suite (measured on the wired stack in the `mayor-et3sl` scout) is the accepted benchmark. Pass-count under a fixed test harness is the direct signal to compare; published per-engine resource claims are themselves rarely measured (jj1vz found zero of six candidates publish an actual RSS number) and are not a valid comparison basis on their own.
