...
use crate::{
    ...
    util::{
        content_type, extract_body, extract_body_quiet, extract_body_recheck,
        parse_resource_version, utc_now_rfc3339,
    },
};
...
pub async fn create_cr<S: Store>(
    ...
    let ct = headers
        ...
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    // extract_body_recheck, not extract_body: create_resource already ran extract_body on this
    // body before falling back here on a lookup miss, so it's already JSON under a stale
    // protobuf Content-Type header — see extract_body_recheck's doc comment.
    let body = extract_body_recheck(&body, ct)?;
...
pub async fn replace_cr<S: Store>(
    ...
    let ct = headers
        ...
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    // extract_body_recheck, not extract_body: replace_resource already ran extract_body on
    // this body before falling back here on a lookup miss, so it's already JSON under a stale
    // protobuf Content-Type header — see extract_body_recheck's doc comment.
    let body = extract_body_recheck(&body, ct)?;
...
pub async fn create_cr_namespaced<S: Store>(
    ...
    let ct = headers
        ...
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    // extract_body_recheck, not extract_body: create_namespaced_resource already ran
    // extract_body on this body before falling back here on a lookup miss, so it's already
    // JSON under a stale protobuf Content-Type header — see extract_body_recheck's doc comment.
    let body = extract_body_recheck(&body, ct)?;
    let obj: serde_json::Value = serde_json::from_slice(&body)
        .map_err(|e| Status::bad_request(format!("invalid JSON: {e}")))?;
...
pub async fn replace_cr_namespaced<S: Store>(
    ...
    let ct = headers
        ...
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
    // extract_body_recheck, not extract_body: replace_namespaced_resource already ran
    // extract_body on this body before falling back here on a lookup miss, so it's already
    // JSON under a stale protobuf Content-Type header — see extract_body_recheck's doc comment.
    let body = extract_body_recheck(&body, ct)?;
    let mut obj: serde_json::Value = serde_json::from_slice(&body)
...
mod tests {
    ...
    /// A ResourceQuota watching a CRD-backed resource via `count/<crd>.<group>` must track
    /// the full create -> delete lifecycle of a custom resource — this is exactly what the
    /// upstream e2e spec "should create a ResourceQuota and capture the life of a custom
    /// resource" polls for. Before this fix, create_cr_namespaced/delete_cr_namespaced never
    /// called update_quota_status at all, so status.used never advanced past whatever it was
    /// (usually unset) regardless of how many custom resources were created or deleted.
    #[tokio::test]
    async fn cr_create_and_delete_updates_count_quota_status_used() {
        let state = make_state();
        install_namespaced_crd(&state).await;

        let quota_key = "/registry/resourcequotas/argocd/cr-quota";
        state
            .store
            .put(
                quota_key,
                Bytes::from(
                    serde_json::json!({
                        "apiVersion": "v1",
                        "kind": "ResourceQuota",
                        "metadata": { "name": "cr-quota", "namespace": "argocd" },
                        "spec": { "hard": { "count/applications.argoproj.io": "1" } }
                    })
                    .to_string(),
                ),
        ...
        assert!(
            ...
            .await
            .is_ok(),
            "create must succeed under the quota's hard limit of 1"
        );

        let stored = state.store.get(quota_key).await.unwrap().unwrap();
        let obj: serde_json::Value = serde_json::from_slice(&stored.value).unwrap();
        assert_eq!(
            obj["status"]["used"]["count/applications.argoproj.io"].as_str(),
            Some("1"),
            "status.used must reflect the just-created custom resource — if \
             create_cr_namespaced stops calling update_quota_status, this stays unset/stale \
             and the e2e spec's poll for 'used: 1' times out"
        );
        ...
        assert_eq!(
            obj["status"]["used"]["count/applications.argoproj.io"].as_str(),
            Some("0"),
            "status.used must decrement back to 0 after the custom resource is deleted — if \
             delete_cr_namespaced stops calling update_quota_status, the quota sticks at 1 \
             forever, permanently blocking any future create once the hard limit was ever hit"
        );
    }
}
