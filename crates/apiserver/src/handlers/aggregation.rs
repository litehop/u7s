...
fn upsert_available_condition(
    ...
    // A non-object `status` can no longer reach the store on either CREATE or UPDATE of the
    // main APIService resource (see
    // `every_create_or_update_operation_webhook_call_clears_status_after_webhook`,
    // resource.rs) or via the `/status` subresource (APIService is registered in
    // `status_dispatch`, whose typed decode rejects a non-object shape outright) — so this
    // no longer needs to defensively coerce `status` back to an object before indexing it.
