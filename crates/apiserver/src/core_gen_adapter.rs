...
// `gen_pod_template_spec_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.core.v1.PodTemplateSpec` descriptor — `metadata`/`spec` delegate to the
// hand-written `gen_object_meta_to_json`/generated `gen_pod_spec_to_json`; see
// `build/codegen.rs::pod_template_spec_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/pod_template_spec_gen.rs"));
...
// `gen_pod_to_json`/`json_to_pod_proto` are generated by `build/codegen.rs` from the
// `.k8s.io.api.core.v1.Pod` descriptor — `metadata` delegates to the hand-written
// `gen_object_meta_to_json` and `spec`/`status` delegate to the already-generated
// `gen_pod_spec_to_json`/`gen_pod_status_to_json`; see `build/codegen.rs::pod_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/pod_gen.rs"));

pub fn decode_pod_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let pod = core_v1::Pod::decode(data).ok()?;
    let mut obj = gen_pod_to_json(pod);
    obj["apiVersion"] = "v1".into();
    obj["kind"] = "Pod".into();
    Some(obj)
}
...
// `gen_podtemplate_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.core.v1.PodTemplate` descriptor — `metadata` delegates to the hand-written
// `gen_object_meta_to_json` and `template` delegates to the generated
// `gen_pod_template_spec_to_json`; see `build/codegen.rs::podtemplate_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/podtemplate_gen.rs"));

pub fn decode_podtemplate_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let pt = core_v1::PodTemplate::decode(data).ok()?;
    let mut obj = gen_podtemplate_to_json(pt);
    obj["apiVersion"] = "v1".into();
    obj["kind"] = "PodTemplate".into();
    Some(obj)
}
...
/// `secrets[]` entries are only emitted once their `name` sub-field is non-empty — an
/// `ObjectReference` with no name is not a usable secret reference a pod could ever mount, the
/// same guard the hand-rolled decoder this replaces used. kind/namespace/uid/apiVersion/
/// resourceVersion/fieldPath are real `ObjectReference` fields a client is entitled to set on a
/// `secrets[]` entry, so this reuses the already-generated `gen_object_reference_to_json`'s full
/// field mapping verbatim rather than re-deriving a partial one — dropping any of them would
/// silently corrupt a GET-modify-PUT round trip. Used by `gen_serviceaccount_to_json`'s `secrets`
/// delegate; see `build/codegen.rs::serviceaccount_delegated_field`.
fn gen_serviceaccount_secret_to_json(r: core_v1::ObjectReference) -> Option<serde_json::Value> {
    if r.name.as_deref().unwrap_or_default().is_empty() {
        return None;
    }
    Some(gen_object_reference_to_json(r))
}

// `gen_serviceaccount_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.core.v1.ServiceAccount` descriptor — `metadata` delegates to the hand-written
// `gen_object_meta_to_json` and `secrets`/`imagePullSecrets` delegate to the hand-written helper
// above/inline closure; see `build/codegen.rs::serviceaccount_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/serviceaccount_gen.rs"));

pub fn decode_serviceaccount_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let sa = core_v1::ServiceAccount::decode(data).ok()?;
    let mut obj = gen_serviceaccount_to_json(sa);
    obj["apiVersion"] = "v1".into();
    obj["kind"] = "ServiceAccount".into();
    Some(obj)
}
...
// `json_to_pod_proto` is defined above, next to `gen_pod_to_json` — see
// `build/codegen.rs::generate_pod`'s doc.
...
mod tests {
    ...
    /// helpers this migration left untouched) and diffs their output against the generated
    /// `gen_pod_to_json`/`json_to_pod_proto`/`gen_podtemplate_to_json`/`gen_serviceaccount_to_json`
    /// this migration installed. A wrong field name or a dropped delegate-table entry in
    /// `build/codegen.rs` would show up here as a JSON/protobuf-byte mismatch even if every
    /// other fixture in this file happens to avoid the exact field it hit.
    #[test]
    fn codegen_migration_pod_family_matches_pre_migration_hand_written_output() {
        fn old_json_to_pod_proto(v: &serde_json::Value) -> core_v1::Pod {
            core_v1::Pod {
        ...
        fn old_decode_pod_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let pod = core_v1::Pod::decode(data).ok()?;
        ...
        fn old_gen_pod_template_spec_to_json(tmpl: core_v1::PodTemplateSpec) -> serde_json::Value {
        ...
        fn old_decode_podtemplate_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let pt = core_v1::PodTemplate::decode(data).ok()?;
            ...
            if let Some(tmpl) = pt.template {
                obj["template"] = old_gen_pod_template_spec_to_json(tmpl);
            } else {
        ...
        fn old_decode_serviceaccount_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
        ...
        // PodSpec itself uses ::sentinel() (already-generated by an earlier phase, untouched by
        // this migration) so every kubelet-observable field — nodeSelector, tolerations,
        // resources, security context, volume mounts, env, affinity, topologySpreadConstraints —
        // is populated; the Pod-level fields around it exercise this migration's own new
        // top-level delegate table.
        let pod = core_v1::Pod {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-pod".to_string()),
                namespace: Some("default".to_string()),
                ..Default::default()
            }),
            spec: Some(core_v1::PodSpec::sentinel()),
            status: Some(core_v1::PodStatus {
                phase: Some("Running".to_string()),
                conditions: vec![core_v1::PodCondition {
                    r#type: Some("Ready".to_string()),
                    status: Some("True".to_string()),
                    ..Default::default()
                }],
        ...
        pod.encode(&mut pod_bytes).unwrap();

        let old_decoded = old_decode_pod_proto_gen(&pod_bytes).expect("old path must decode");
        let new_decoded = decode_pod_proto_gen(&pod_bytes).expect("new path must decode");
        assert_eq!(
            old_decoded, new_decoded,
            "codegen-generated decode_pod_proto_gen must match the pre-migration hand-written \
             decoder field-for-field — any divergence here is a silent kubelet-observable field \
             drop introduced by the Phase 3.5 migration"
        );

        let old_encoded = old_json_to_pod_proto(&new_decoded).encode_to_vec();
        let new_encoded = encode_pod_proto_gen(&new_decoded);
        assert_eq!(
            old_encoded, new_encoded,
            "codegen-generated encode_pod_proto_gen must produce byte-identical protobuf to the \
             pre-migration hand-written encoder — kubelet re-reads exactly these bytes to \
             actuate containers, so any divergence here is a silent wire-format regression"
        );

        let podtemplate = core_v1::PodTemplate {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-template".to_string()),
                ..Default::default()
            }),
            template: Some(core_v1::PodTemplateSpec {
                metadata: Some(meta_v1::ObjectMeta {
                    labels: [("app".to_string(), "audit".to_string())]
                        .into_iter()
                        .collect(),
                    ..Default::default()
                }),
        ...
        let mut podtemplate_bytes = Vec::new();
        podtemplate.encode(&mut podtemplate_bytes).unwrap();
        assert_eq!(
            old_decode_podtemplate_proto_gen(&podtemplate_bytes),
            decode_podtemplate_proto_gen(&podtemplate_bytes),
            "codegen-generated decode_podtemplate_proto_gen must match the pre-migration \
             hand-written decoder field-for-field"
        );

        let empty_podtemplate = core_v1::PodTemplate {
            metadata: Some(meta_v1::ObjectMeta::default()),
            template: None,
        };
        let mut empty_bytes = Vec::new();
        empty_podtemplate.encode(&mut empty_bytes).unwrap();
        assert_eq!(
            old_decode_podtemplate_proto_gen(&empty_bytes),
            decode_podtemplate_proto_gen(&empty_bytes),
            "a PodTemplate with no template must still round-trip an empty-object \"template\" \
             key, matching the pre-migration decoder exactly"
        );

        let service_account = core_v1::ServiceAccount {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-sa".to_string()),
                namespace: Some("default".to_string()),
                ..Default::default()
            }),
            secrets: vec![
                core_v1::ObjectReference {
                    ...
                    namespace: Some("default".to_string()),
                    name: Some("audit-sa-token".to_string()),
                    uid: Some("uid-1".to_string()),
                    api_version: Some("v1".to_string()),
                    resource_version: Some("42".to_string()),
                    field_path: Some("data".to_string()),
                },
                // A secrets[] entry with no name is dropped entirely, not emitted as a
                // name-less object — the pre-migration decoder's own filter_map behavior.
                core_v1::ObjectReference {
                    kind: Some("Secret".to_string()),
                    ..Default::default()
                },
        ...
        service_account.encode(&mut sa_bytes).unwrap();
        assert_eq!(
            old_decode_serviceaccount_proto_gen(&sa_bytes),
            decode_serviceaccount_proto_gen(&sa_bytes),
            "codegen-generated decode_serviceaccount_proto_gen must match the pre-migration \
             hand-written decoder field-for-field, including the name-presence filter on \
             secrets[]"
        );
    }
