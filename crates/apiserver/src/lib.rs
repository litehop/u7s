...
pub async fn run(args: Args) -> anyhow::Result<()> {
    ...
    // against serve_tls below instead of being discarded. This fatal-at-boot behavior is
    // unchanged by the SIGHUP-triggered reload spawned right below, which reuses the same
    // manifest directory but with log-and-skip semantics instead (mayor-bh36n).
