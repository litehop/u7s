...
/// metrics client gets a 404 from `/apis/metrics.k8s.io/v1beta1/...` (nothing serves that
/// group at all) and the HPA's `status.conditions[ScalingActive]` never becomes True.
///
/// Adapted from upstream metrics-server's own `components.yaml` (v0.9.0, the latest stable
/// release as of this writing) — ServiceAccount, both bootstrap ClusterRoles, the RoleBinding
/// to the pre-existing `extension-apiserver-authentication-reader` Role (see the
/// `system:auth-delegator` block above), both ClusterRoleBindings, Service, Deployment, and
...
///     signs both the kubelet's serving cert and metrics-server's trust bundle; here every
///     run generates a fresh, self-signed kubelet cert with no shared CA to validate it
...
async fn seed_metrics_server(
    ...
    // kube-system/metrics-server ServiceAccount — the pod authenticates to us as this SA
    // when it calls TokenReview/SubjectAccessReview to validate its own callers (see the
    // ClusterRoleBindings below).
    let sa_key = keys::object_key("serviceaccounts", "kube-system", "metrics-server");
    let sa_body = serde_json::json!({
        "apiVersion": "v1",
        "kind": "ServiceAccount",
        "metadata": {
            "name": "metrics-server",
            "namespace": "kube-system",
            "uid": "00000000-0000-0000-0000-000000000070",
            "creationTimestamp": TS,
    ...
    match store
        ...
        .await
    {
        Ok(_) => tracing::info!("seeded ServiceAccount: kube-system/metrics-server"),
        Err(u7s_store::StoreError::AlreadyExists { .. }) => {}
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed ServiceAccount kube-system/metrics-server: {e}"
            ))
        }
    }

    // ClusterRole system:aggregated-metrics-reader — aggregates into the default
    // admin/edit/view ClusterRoles so ordinary users can `kubectl top` without a dedicated
    // grant, matching upstream's own manifest.
    let reader_key = keys::group_object_key(
        RBAC_GROUP,
        "clusterroles",
        None,
        "system:aggregated-metrics-reader",
    );
    let reader_body = serde_json::json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "ClusterRole",
        "metadata": {
            "name": "system:aggregated-metrics-reader",
            "uid": "00000000-0000-0000-0000-000000000071",
            "creationTimestamp": TS,
            "labels": {
    ...
    match store
        .put(&reader_key, Bytes::from(reader_body.to_string()), None)
        .await
    {
        Ok(_) => tracing::info!("seeded ClusterRole: system:aggregated-metrics-reader"),
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed ClusterRole system:aggregated-metrics-reader: {e}"
            ))
        }
    }

    // ClusterRole system:metrics-server — what metrics-server itself needs: read nodes/pods
    // to know what to scrape, plus the nodes/metrics subresource for the kubelet stats API.
    let server_role_key =
        keys::group_object_key(RBAC_GROUP, "clusterroles", None, "system:metrics-server");
    let server_role_body = serde_json::json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "ClusterRole",
        "metadata": {
            "name": "system:metrics-server",
            "uid": "00000000-0000-0000-0000-000000000072",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "rules": [
    ...
    match store
        .put(
            &server_role_key,
            Bytes::from(server_role_body.to_string()),
            None,
        )
        .await
    {
        Ok(_) => tracing::info!("seeded ClusterRole: system:metrics-server"),
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed ClusterRole system:metrics-server: {e}"
            ))
        }
    }

    // RoleBinding kube-system/metrics-server-auth-reader — binds metrics-server's SA to the
    // pre-existing extension-apiserver-authentication-reader Role (seeded above, alongside
    // system:auth-delegator).
    let auth_reader_binding_key = keys::group_object_key(
        RBAC_GROUP,
        "rolebindings",
        Some("kube-system"),
        "metrics-server-auth-reader",
    );
    let auth_reader_binding_body = serde_json::json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "RoleBinding",
        "metadata": {
            "name": "metrics-server-auth-reader",
            "namespace": "kube-system",
            "uid": "00000000-0000-0000-0000-000000000073",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "roleRef": {
    ...
    match store
        .put(
            &auth_reader_binding_key,
            Bytes::from(auth_reader_binding_body.to_string()),
            None,
        )
        .await
    {
        Ok(_) => tracing::info!("seeded RoleBinding: kube-system/metrics-server-auth-reader"),
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed RoleBinding kube-system/metrics-server-auth-reader: {e}"
            ))
        }
    }

    // ClusterRoleBinding metrics-server:system:auth-delegator — lets metrics-server delegate
    // authn/authz decisions for its OWN callers back to us via TokenReview/SubjectAccessReview
    // (see the system:auth-delegator ClusterRole's doc comment above for why this exists).
    let auth_delegator_binding_key = keys::group_object_key(
        RBAC_GROUP,
        "clusterrolebindings",
        None,
        "metrics-server:system:auth-delegator",
    );
    let auth_delegator_binding_body = serde_json::json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "ClusterRoleBinding",
        "metadata": {
            "name": "metrics-server:system:auth-delegator",
            "uid": "00000000-0000-0000-0000-000000000074",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "roleRef": {
    ...
    match store
        .put(
            &auth_delegator_binding_key,
            Bytes::from(auth_delegator_binding_body.to_string()),
            None,
        )
        .await
    {
        Ok(_) => tracing::info!("seeded ClusterRoleBinding: metrics-server:system:auth-delegator"),
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed ClusterRoleBinding metrics-server:system:auth-delegator: {e}"
            ))
        }
    }

    // ClusterRoleBinding system:metrics-server — binds metrics-server's SA to its own
    // ClusterRole above.
    let server_binding_key = keys::group_object_key(
        RBAC_GROUP,
        "clusterrolebindings",
        None,
        "system:metrics-server",
    );
    let server_binding_body = serde_json::json!({
        "apiVersion": "rbac.authorization.k8s.io/v1",
        "kind": "ClusterRoleBinding",
        "metadata": {
            "name": "system:metrics-server",
            "uid": "00000000-0000-0000-0000-000000000075",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "roleRef": {
    ...
    match store
        .put(
            &server_binding_key,
            Bytes::from(server_binding_body.to_string()),
            None,
        )
        .await
    {
        Ok(_) => tracing::info!("seeded ClusterRoleBinding: system:metrics-server"),
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed ClusterRoleBinding system:metrics-server: {e}"
            ))
        }
    }

    // kube-system/metrics-server Service — the aggregation proxy's spec.service target.
    // This bypasses the normal create-Service handler that would otherwise allocate and
    // reserve a clusterIP, so derive a fixed host from the configured CIDR the same way
    // seed_services does for kube-dns (host 10) — host 11 here, one past it.
    let range = if service_cluster_ip_range.is_empty() {
        "10.96.0.0/12"
    } else {
        service_cluster_ip_range
    };
    let metrics_server_ip = state::ServiceIpAllocator::from_cidr(range)
        .map_err(|e| anyhow::anyhow!("invalid --service-cluster-ip-range '{range}': {e}"))?
        .nth_host(11)
        .map_err(|e| {
            anyhow::anyhow!(
                "--service-cluster-ip-range '{range}' too small for metrics-server Service: {e}"
            )
        })?;
    let svc_key = keys::object_key("services", "kube-system", "metrics-server");
    let svc_body = serde_json::json!({
        ...
        "metadata": {
            "name": "metrics-server",
            "namespace": "kube-system",
            "uid": "00000000-0000-0000-0000-000000000076",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "spec": {
            "clusterIP": metrics_server_ip.to_string(),
    ...
    match store
        .put(&svc_key, Bytes::from(svc_body.to_string()), Some(0))
        .await
    {
        Ok(_) => tracing::info!("seeded Service: kube-system/metrics-server"),
        Err(u7s_store::StoreError::AlreadyExists { .. }) => {}
        Err(e) => {
            return Err(anyhow::anyhow!(
    ...
    // kube-system/metrics-server Deployment — scrapes each kubelet's stats endpoint and
    // serves the results as the metrics.k8s.io API.
    let deploy_key =
        keys::group_object_key("apps", "deployments", Some("kube-system"), "metrics-server");
    ...
    crate::handlers::defaults::apply_defaults("apps", "deployments", &mut deploy_body);
    match store
        .put(&deploy_key, Bytes::from(deploy_body.to_string()), Some(0))
        .await
    {
        Ok(_) => tracing::info!("seeded Deployment: kube-system/metrics-server"),
        Err(u7s_store::StoreError::AlreadyExists { .. }) => {}
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed Deployment kube-system/metrics-server: {e}"
            ))
        }
    }

    // APIService v1beta1.metrics.k8s.io — registers the group with the aggregation proxy
    // (handlers::aggregation::proxy_middleware), which forwards /apis/metrics.k8s.io/v1beta1/*
    // to this Service. insecureSkipTLSVerify is required: metrics-server generates its own
    // self-signed serving cert at startup (--cert-dir=/tmp above), and there is no shared CA
    // for u7s to validate it against.
    let apiservice_key = keys::group_object_key(
        "apiregistration.k8s.io",
        "apiservices",
        None,
        "v1beta1.metrics.k8s.io",
    );
    let apiservice_body = serde_json::json!({
        "apiVersion": "apiregistration.k8s.io/v1",
        "kind": "APIService",
        "metadata": {
            "name": "v1beta1.metrics.k8s.io",
            "uid": "00000000-0000-0000-0000-000000000078",
            "creationTimestamp": TS,
            "labels": { "k8s-app": "metrics-server" }
        },
        "spec": {
    ...
    match store
        .put(
            &apiservice_key,
            Bytes::from(apiservice_body.to_string()),
            Some(0),
        )
        .await
    {
        Ok(_) => tracing::info!("seeded APIService: v1beta1.metrics.k8s.io"),
        Err(u7s_store::StoreError::AlreadyExists { .. }) => {}
        Err(e) => {
            return Err(anyhow::anyhow!(
                "seed APIService v1beta1.metrics.k8s.io: {e}"
            ))
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn seed_metrics_server_deployment_uses_kubelet_insecure_tls() {
        // Regression test: this lab's kubelet serving cert is self-signed per run with no
        // shared CA metrics-server could otherwise be given to trust it against. Without
        // --kubelet-insecure-tls, metrics-server fails every kubelet scrape with a TLS
        // handshake error and `kubectl top` / the HPA controller never see real metric
        // values, even though the APIService itself is registered and reachable. Revert the
        // flag from the Deployment's container args and this test fails again.
        let store = make_store();
        seed_metrics_server(&store, "10.96.0.0/12")
            .await
            .expect("seed must not fail");
        ...
        let obj = store
            ...
            .await
            .expect("get must not fail")
            .expect("Deployment kube-system/metrics-server must exist after seeding");
        let parsed: serde_json::Value = serde_json::from_slice(&obj.value).expect("valid json");
        assert_eq!(parsed["kind"].as_str(), Some("Deployment"));
        let args = parsed["spec"]["template"]["spec"]["containers"][0]["args"]
            .as_array()
            .expect("container args must be an array");
        let args: Vec<&str> = args.iter().filter_map(|a| a.as_str()).collect();
        assert!(
            args.contains(&"--kubelet-insecure-tls"),
            "metrics-server must be started with --kubelet-insecure-tls, or every scrape of \
             this lab's self-signed kubelet cert fails TLS verification: got args {args:?}"
        );
    }

    #[tokio::test]
    async fn seed_metrics_server_registers_apiservice_pointing_at_kube_system_service() {
        // Regression test: without this APIService registered, handlers::aggregation's
        // proxy_middleware has nothing to match /apis/metrics.k8s.io/v1beta1/* against, and
        // every HPA CPU/memory target request 404s exactly like it did before metrics-server
        // existed (the original bug this seed function fixes). insecureSkipTLSVerify must be
        // true because metrics-server generates its own self-signed serving cert at startup,
        // which no shared CA in this cluster can validate.
        let store = make_store();
        seed_metrics_server(&store, "10.96.0.0/12")
            .await
            .expect("seed must not fail");
        ...
        let obj = store
            ...
            .await
            .expect("get must not fail")
            .expect("APIService v1beta1.metrics.k8s.io must exist after seeding");
        let parsed: serde_json::Value = serde_json::from_slice(&obj.value).expect("valid json");
        assert_eq!(parsed["spec"]["group"].as_str(), Some("metrics.k8s.io"));
        assert_eq!(parsed["spec"]["version"].as_str(), Some("v1beta1"));
        assert_eq!(
            parsed["spec"]["service"]["name"].as_str(),
            Some("metrics-server")
        );
        assert_eq!(
            parsed["spec"]["service"]["namespace"].as_str(),
            Some("kube-system")
        );
        assert_eq!(
            parsed["spec"]["insecureSkipTLSVerify"].as_bool(),
            Some(true),
            "insecureSkipTLSVerify must be true — metrics-server's self-signed cert has no \
             CA in this cluster for the aggregation proxy to validate it against"
        );
    }

    #[tokio::test]
    async fn seed_metrics_server_is_idempotent() {
        // A second call must not error — startup can happen against an existing database.
        // Without idempotency, every apiserver restart after the first boot would fail.
        let store = make_store();
        seed_metrics_server(&store, "10.96.0.0/12")
            .await
            .expect("first seed must not fail");
        ...
        let key = keys::object_key("services", "kube-system", "metrics-server");
        let obj = store.get(&key).await.expect("get must not fail");
        assert!(
            obj.is_some(),
            "Service kube-system/metrics-server must still exist after second seed call"
        );
    }
