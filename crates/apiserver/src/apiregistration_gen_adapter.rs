...
/// `type`/`status`/`reason`/`message` are only emitted when non-empty, and `lastTransitionTime`
/// only when its seconds are non-zero — matching the aggregator availability controller's own
/// convention (`handlers/aggregation.rs::upsert_available_condition`) of only ever setting fields
/// it has real values for; unlike e.g. `gen_namespace_condition_to_json`'s `type`/`status`, which
/// are unconditional for a different upstream Kind. Used by `gen_apiservice_status_to_json`'s
/// `conditions` delegate; see `build/codegen.rs::apiservice_status_delegated_field`.
fn gen_apiservice_condition_to_json(
    c: apiregistration_v1::ApiServiceCondition,
) -> serde_json::Value {
...
// `gen_apiservice_spec_to_json`/`gen_apiservice_status_to_json`/`gen_apiservice_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.kube_aggregator.pkg.apis.apiregistration.v1`
// descriptors — `metadata` delegates to the hand-written `gen_object_meta_to_json` above,
// `spec.caBundle` (a `bytes` field) delegates to an inline base64 encode, and `status.conditions`
// delegates to the hand-written `gen_apiservice_condition_to_json` above (its `lastTransitionTime`
// needs RFC3339 conversion the mechanical walker can't derive from the schema alone); see
// `build/codegen.rs::apiservice_spec_delegated_field`/`apiservice_status_delegated_field`/
// `apiservice_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/apiservice_spec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/apiservice_status_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/apiservice_gen.rs"));
...
pub fn decode_apiservice_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut obj = gen_apiservice_to_json(svc);
    obj["apiVersion"] = "apiregistration.k8s.io/v1".into();
    obj["kind"] = "APIService".into();
    Some(obj)
}
...
mod tests {
    ...
    /// `spec.caBundle`, and `status.conditions[].lastTransitionTime`). A wrong field name or a
    /// dropped delegate-table entry in `build/codegen.rs` would show up here as a JSON mismatch
    /// even if the other decode tests above happen to avoid the exact field it hit.
    #[test]
    fn codegen_migration_apiservice_matches_pre_migration_hand_written_output() {
        fn old_decode_apiservice_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let svc = apiregistration_v1::ApiService::decode(data).ok()?;
        ...
        let old_decoded = old_decode_apiservice_proto_gen(&buf).expect("old path must decode");
        let new_decoded = decode_apiservice_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old_decoded, new_decoded,
            "codegen-generated decode_apiservice_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent \
             aggregation-layer field drop (e.g. spec.service routing info or the \
             status.conditions availability signal) introduced by this migration"
        );
    }
}
