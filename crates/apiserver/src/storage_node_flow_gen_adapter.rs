...
// `gen_csinode_driver_to_json`/`gen_csinode_to_json` are generated by `build/codegen.rs` from the
// `.k8s.io.api.storage.v1.CSINodeDriver`/`.k8s.io.api.storage.v1.CSINode` descriptors — `metadata`
// delegates to `gen_object_meta_to_json` and `spec` delegates to a hand-written template that
// always emits `{"drivers": [...]}` (even when empty), matching the pre-migration decoder this
// replaces exactly; see `build/codegen.rs::csinode_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/csinode_driver_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/csinode_gen.rs"));

pub fn decode_csinode_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = storage_v1::CsiNode::decode(data).ok()?;
    let mut result = gen_csinode_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "CSINode".into();
    Some(result)
}

// ---- Decoder A: CSIDriver ------------------------------------------------

// `gen_token_request_to_json`/`gen_csidriverspec_to_json`/`gen_csidriver_to_json` are generated by
// `build/codegen.rs` from the `.k8s.io.api.storage.v1.TokenRequest`/`CSIDriverSpec`/`CSIDriver`
// descriptors — `metadata` delegates to `gen_object_meta_to_json`, `spec` delegates wholesale to
// `gen_csidriverspec_to_json` (always emitted, even `{}`), and `tokenRequests`/
// `nodeAllocatableUpdatePeriodSeconds` delegate to their own zero-filtering templates; see
// `build/codegen.rs::token_request_delegated_field`/`csidriverspec_delegated_field`/
// `csidriver_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/token_request_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/csidriverspec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/csidriver_gen.rs"));

pub fn decode_csidriver_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = storage_v1::CsiDriver::decode(data).ok()?;
    let mut result = gen_csidriver_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "CSIDriver".into();
    Some(result)
}

// ---- Decoder A: CSIStorageCapacity ----------------------------------------

// `gen_csistoragecapacity_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.storage.v1.CSIStorageCapacity` descriptor — `metadata` delegates to
// `gen_object_meta_to_json`, `nodeTopology` delegates to `gen_label_selector_to_json`, and
// `capacity`/`maximumVolumeSize` are handled by the mechanical walker's own `Quantity` special
// case; see `build/codegen.rs::csistoragecapacity_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/csistoragecapacity_gen.rs"));
...
pub fn decode_csistoragecapacity_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut result = gen_csistoragecapacity_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "CSIStorageCapacity".into();
    Some(result)
}

// ---- Decoder A: VolumeAttachment ------------------------------------------

// `gen_volume_error_to_json`/`gen_volumeattachmentspec_to_json`/`gen_volumeattachmentstatus_to_json`/
// `gen_volumeattachment_to_json` are generated by `build/codegen.rs` from the
// `.k8s.io.api.storage.v1.VolumeError`/`VolumeAttachmentSpec`/`VolumeAttachmentStatus`/
// `VolumeAttachment` descriptors — `metadata` delegates to `gen_object_meta_to_json`, `spec`
// delegates wholesale to `gen_volumeattachmentspec_to_json` (whose own `source` field is always
// emitted, even `{}`), and `status`/`attachError`/`detachError` delegate to their own
// non-empty-gated templates; see `build/codegen.rs::volume_error_delegated_field`/
// `volumeattachmentspec_delegated_field`/`volumeattachmentstatus_delegated_field`/
// `volumeattachment_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/volume_error_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/volumeattachmentspec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/volumeattachmentstatus_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/volumeattachment_gen.rs"));
...
pub fn decode_volumeattachment_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut result = gen_volumeattachment_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "VolumeAttachment".into();
    Some(result)
}

// ---- Decoder A: StorageClass -----------------------------------------------

// `gen_storageclass_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.storage.v1.StorageClass` descriptor — `metadata` delegates to
// `gen_object_meta_to_json` and `allowedTopologies` delegates to a hand-written template
// (`TopologySelectorTerm.matchLabelExpressions` is always emitted per element, even `[]`); see
// `build/codegen.rs::storageclass_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/storageclass_gen.rs"));

pub fn decode_storageclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = storage_v1::StorageClass::decode(data).ok()?;
    let mut result = gen_storageclass_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "StorageClass".into();
    Some(result)
}

// ---- Decoder A: VolumeAttributesClass --------------------------------------

// `gen_volumeattributesclass_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.storage.v1.VolumeAttributesClass` descriptor — `metadata` delegates to
// `gen_object_meta_to_json`; see `build/codegen.rs::volumeattributesclass_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/volumeattributesclass_gen.rs"));
...
pub fn decode_volumeattributesclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut result = gen_volumeattributesclass_to_json(obj);
    result["apiVersion"] = "storage.k8s.io/v1".into();
    result["kind"] = "VolumeAttributesClass".into();
    Some(result)
}

// ---- Decoder A: RuntimeClass -----------------------------------------------

// `gen_runtimeclass_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.node.v1.RuntimeClass` descriptor — `metadata` delegates to
// `gen_object_meta_to_json`, `handler` always emits (defaulting unset to `""`), and `overhead`
// delegates to a hand-written template that filters `podFixed`'s quantities before deciding
// whether to emit the key at all; see `build/codegen.rs::runtimeclass_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/runtimeclass_gen.rs"));

pub fn decode_runtimeclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = node_v1::RuntimeClass::decode(data).ok()?;
    let mut result = gen_runtimeclass_to_json(obj);
    result["apiVersion"] = "node.k8s.io/v1".into();
    result["kind"] = "RuntimeClass".into();
    Some(result)
}

// ---- Decoder A: PriorityClass (scheduling.k8s.io/v1) ----------------------

// `gen_priorityclass_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.scheduling.v1.PriorityClass` descriptor — `metadata` delegates to
// `gen_object_meta_to_json`, `value` always emits (defaulting unset to `0`), and `globalDefault`
// only emits when explicitly `true`; see `build/codegen.rs::priorityclass_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/priorityclass_gen.rs"));

pub fn decode_priorityclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = scheduling_v1::PriorityClass::decode(data).ok()?;
    let mut result = gen_priorityclass_to_json(obj);
    result["apiVersion"] = "scheduling.k8s.io/v1".into();
    result["kind"] = "PriorityClass".into();
    Some(result)
}

// ---- Decoder A: FlowSchema -------------------------------------------------

// `gen_flowcontrol_subject_to_json`/`gen_resource_policy_rule_to_json`/
// `gen_policy_rules_with_subjects_to_json`/`gen_flowschema_condition_to_json`/
// `gen_flowschemaspec_to_json`/`gen_flowschemastatus_to_json`/`gen_flowschema_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.api.flowcontrol.v1.Subject`/
// `ResourcePolicyRule`/`PolicyRulesWithSubjects`/`FlowSchemaCondition`/`FlowSchemaSpec`/
// `FlowSchemaStatus`/`FlowSchema` descriptors — `metadata` delegates to `gen_object_meta_to_json`,
// `serviceAccount`/`clusterScope`/`lastTransitionTime` delegate to their own hand-written
// templates, and every spec/status/rule wrapper delegates wholesale to the layer below it; see
// `build/codegen.rs::flowcontrol_subject_delegated_field`/
// `resource_policy_rule_delegated_field`/`policy_rules_with_subjects_delegated_field`/
// `flowschema_condition_delegated_field`/`flowschemaspec_delegated_field`/
// `flowschemastatus_delegated_field`/`flowschema_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/flowcontrol_subject_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/resource_policy_rule_gen.rs"));
include!(concat!(
    env!("OUT_DIR"),
    "/policy_rules_with_subjects_gen.rs"
));
include!(concat!(env!("OUT_DIR"), "/flowschema_condition_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/flowschemaspec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/flowschemastatus_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/flowschema_gen.rs"));

pub fn decode_flowschema_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let obj = flowcontrol_v1::FlowSchema::decode(data).ok()?;
    let mut result = gen_flowschema_to_json(obj);
    result["apiVersion"] = "flowcontrol.apiserver.k8s.io/v1".into();
    result["kind"] = "FlowSchema".into();
    Some(result)
}

// ---- Decoder A: PriorityLevelConfiguration ---------------------------------

// `gen_queuing_configuration_to_json`/`gen_limit_response_to_json`/
// `gen_limited_priority_level_configuration_to_json`/
// `gen_exempt_priority_level_configuration_to_json`/
// `gen_priority_level_configuration_condition_to_json`/
// `gen_prioritylevelconfigurationspec_to_json`/`gen_prioritylevelconfigurationstatus_to_json`/
// `gen_prioritylevelconfiguration_to_json` are generated by `build/codegen.rs` from the
// `.k8s.io.api.flowcontrol.v1.QueuingConfiguration`/`LimitResponse`/
// `LimitedPriorityLevelConfiguration`/`ExemptPriorityLevelConfiguration`/
// `PriorityLevelConfigurationCondition`/`PriorityLevelConfigurationSpec`/
// `PriorityLevelConfigurationStatus`/`PriorityLevelConfiguration` descriptors — `metadata`
// delegates to `gen_object_meta_to_json`, every `nominalConcurrencyShares`/`lendablePercent`/
// `queues`/`handSize`/`queueLengthLimit` delegates to a zero-filtering template, and every
// spec/status/nested-config wrapper delegates wholesale to the layer below it, only inserted once
// non-empty; see `build/codegen.rs::queuing_configuration_delegated_field`/
// `limit_response_delegated_field`/`limited_priority_level_configuration_delegated_field`/
...
include!(concat!(env!("OUT_DIR"), "/queuing_configuration_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/limit_response_gen.rs"));
include!(concat!(
    env!("OUT_DIR"),
    "/limited_priority_level_configuration_gen.rs"
));
include!(concat!(
    env!("OUT_DIR"),
    "/exempt_priority_level_configuration_gen.rs"
));
include!(concat!(
    env!("OUT_DIR"),
    "/priority_level_configuration_condition_gen.rs"
));
include!(concat!(
    env!("OUT_DIR"),
    "/prioritylevelconfigurationspec_gen.rs"
));
include!(concat!(
    env!("OUT_DIR"),
    "/prioritylevelconfigurationstatus_gen.rs"
));
include!(concat!(
    env!("OUT_DIR"),
    "/prioritylevelconfiguration_gen.rs"
));
...
pub fn decode_prioritylevelconfiguration_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut result = gen_prioritylevelconfiguration_to_json(obj);
    result["apiVersion"] = "flowcontrol.apiserver.k8s.io/v1".into();
    result["kind"] = "PriorityLevelConfiguration".into();
    Some(result)
}
...
mod tests {
    ...
    // ---- Byte-identical audit: codegen migration (Phase 4.6) ----
    //
    // Reconstructs every pre-migration hand-written `decode_*_proto_gen` function in this file
    // verbatim (each `old_decode_*` below is a byte-for-byte copy of the body this migration
    // deleted) and diffs its output against the codegen-generated replacement on a
    // `Sentinel::sentinel()`-populated input for each of the ten migrated types. A wrong JSON key,
    // a missing zero/empty-elision guard, or a dropped delegate in `build/codegen.rs` would show
    // up here as a mismatch — silently misdescribing a StorageClass/RuntimeClass/FlowSchema/etc.
    // to kubectl or to a controller watching these resources is exactly the failure mode a
    // byte-for-byte diff against the known-good pre-migration output catches.
    #[test]
    fn codegen_migration_storage_node_flow_family_matches_pre_migration_hand_written_output() {
        fn old_decode_csinode_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::CsiNode::decode(data).ok()?;
        ...
        fn old_decode_csidriver_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::CsiDriver::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_gen_quantity_opt_to_json(
        ...
        fn old_decode_csistoragecapacity_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::CsiStorageCapacity::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
            ...
            if let Some(v) = old_gen_quantity_opt_to_json(obj.capacity) {
                result["capacity"] = v;
            }
            if let Some(v) = old_gen_quantity_opt_to_json(obj.maximum_volume_size) {
        ...
        fn old_decode_volumeattachment_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::VolumeAttachment::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_decode_storageclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::StorageClass::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_decode_volumeattributesclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = storage_v1::VolumeAttributesClass::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_decode_runtimeclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = node_v1::RuntimeClass::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_decode_priorityclass_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = scheduling_v1::PriorityClass::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        fn old_gen_policy_rules_with_subjects_to_json(
        ...
        fn old_gen_flowschema_condition_to_json(
            c: flowcontrol_v1::FlowSchemaCondition,
        ) -> serde_json::Value {
            let mut m = serde_json::Map::new();
        ...
        fn old_decode_flowschema_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let obj = flowcontrol_v1::FlowSchema::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
            ...
            if let Some(spec) = obj.spec {
                ...
                if !spec.rules.is_empty() {
                    let rules: Vec<serde_json::Value> = spec
                        .rules
                        .into_iter()
                        .map(old_gen_policy_rules_with_subjects_to_json)
                        .collect();
            ...
            if let Some(status) = obj.status {
                if !status.conditions.is_empty() {
                    let conds: Vec<serde_json::Value> = status
                        .conditions
                        .into_iter()
                        .map(old_gen_flowschema_condition_to_json)
                        .collect();
        ...
        fn old_decode_prioritylevelconfiguration_proto_gen(
            data: &[u8],
        ) -> Option<serde_json::Value> {
            let obj = flowcontrol_v1::PriorityLevelConfiguration::decode(data).ok()?;
            let meta = gen_object_meta_to_json(obj.metadata.unwrap_or_default());
        ...
        // Every fixture below is `Sentinel::sentinel()` on the whole top-level message (or, for
        // VolumeAttachment, one sentinel per mutually-exclusive `source` variant) rather than a
        // hand-picked subset of fields — every scalar this migration's mechanical walker or a
        // hand-written delegate could mishandle (a missing zero-filter, a wrong true-only guard, a
        // dropped RFC3339 conversion) is exercised in a single encode/decode round trip.

        macro_rules! assert_matches_pre_migration {
            ($obj:expr, $old:expr, $new:expr, $label:literal) => {{
                let mut buf = Vec::new();
                $obj.encode(&mut buf).unwrap();
                assert_eq!(
                    $old(&buf),
                    $new(&buf),
                    "{}: codegen-generated decoder must match the pre-migration hand-written \
                     decoder field-for-field — any divergence here is a silent field drop or \
                     mis-encoding introduced by the Phase 4.6 migration",
                    $label
                );
            }};
        }

        assert_matches_pre_migration!(
            storage_v1::CsiNode::sentinel(),
            old_decode_csinode_proto_gen,
            decode_csinode_proto_gen,
            "CSINode"
        );
        assert_matches_pre_migration!(
            storage_v1::CsiDriver::sentinel(),
            old_decode_csidriver_proto_gen,
            decode_csidriver_proto_gen,
            "CSIDriver"
        );
        assert_matches_pre_migration!(
            storage_v1::CsiStorageCapacity::sentinel(),
            old_decode_csistoragecapacity_proto_gen,
            decode_csistoragecapacity_proto_gen,
            "CSIStorageCapacity"
        );
        assert_matches_pre_migration!(
            storage_v1::StorageClass::sentinel(),
            old_decode_storageclass_proto_gen,
            decode_storageclass_proto_gen,
            "StorageClass"
        );
        assert_matches_pre_migration!(
            storage_v1::VolumeAttributesClass::sentinel(),
            old_decode_volumeattributesclass_proto_gen,
            decode_volumeattributesclass_proto_gen,
            "VolumeAttributesClass"
        );
        assert_matches_pre_migration!(
            node_v1::RuntimeClass::sentinel(),
            old_decode_runtimeclass_proto_gen,
            decode_runtimeclass_proto_gen,
            "RuntimeClass"
        );
        assert_matches_pre_migration!(
            scheduling_v1::PriorityClass::sentinel(),
            old_decode_priorityclass_proto_gen,
            decode_priorityclass_proto_gen,
            "PriorityClass"
        );
        assert_matches_pre_migration!(
            flowcontrol_v1::FlowSchema::sentinel(),
            old_decode_flowschema_proto_gen,
            decode_flowschema_proto_gen,
            "FlowSchema"
        );
        assert_matches_pre_migration!(
            flowcontrol_v1::PriorityLevelConfiguration::sentinel(),
            old_decode_prioritylevelconfiguration_proto_gen,
            decode_prioritylevelconfiguration_proto_gen,
            "PriorityLevelConfiguration"
        );

        // VolumeAttachmentSource's two members are mutually exclusive on a real object (upstream:
        // "Exactly one member can be set"), so `VolumeAttachment::sentinel()` (which populates
        // both) is a fixture no real client would ever send. Audit both real variants instead —
        // `persistentVolumeName` set, and `inlineVolumeSpec` set — each still built from
        // `Sentinel::sentinel()` for every other field on the message.
        let persistent_volume_variant = storage_v1::VolumeAttachment {
            spec: Some(storage_v1::VolumeAttachmentSpec {
                source: Some(storage_v1::VolumeAttachmentSource {
                    persistent_volume_name: Some("__sentinel__".to_string()),
                    inline_volume_spec: None,
                }),
                ..storage_v1::VolumeAttachmentSpec::sentinel()
            }),
            ..storage_v1::VolumeAttachment::sentinel()
        };
        assert_matches_pre_migration!(
            persistent_volume_variant,
            old_decode_volumeattachment_proto_gen,
            decode_volumeattachment_proto_gen,
            "VolumeAttachment (persistentVolumeName variant)"
        );

        let inline_volume_spec_variant = storage_v1::VolumeAttachment {
            spec: Some(storage_v1::VolumeAttachmentSpec {
                source: Some(storage_v1::VolumeAttachmentSource {
                    persistent_volume_name: None,
                    inline_volume_spec: Some(
        ...
        assert_matches_pre_migration!(
            inline_volume_spec_variant,
            old_decode_volumeattachment_proto_gen,
            decode_volumeattachment_proto_gen,
            "VolumeAttachment (inlineVolumeSpec variant)"
        );
    }
}
