...
// `gen_statefulset_spec_to_json`/`gen_statefulset_status_to_json`/`gen_statefulset_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.api.apps.v1.StatefulSetSpec`/
// `StatefulSetStatus`/`StatefulSet` descriptors — `metadata` delegates to `gen_object_meta_to_json`,
// `selector`/`template` delegate wholesale to `gen_apps_spec_to_json` above, `updateStrategy`/
// `persistentVolumeClaimRetentionPolicy`/`ordinals`/`volumeClaimTemplates` delegate to hand-written
// blocks (IntOrString/PersistentVolumeClaim handling the mechanical walker can't derive from the
// schema alone), and `conditions` delegates to the `apps_condition_to_json!` macro above; see
// `build/codegen.rs::statefulset_spec_delegated_field`/`statefulset_status_delegated_field`/
// `statefulset_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/statefulset_spec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/statefulset_status_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/statefulset_gen.rs"));

pub fn decode_statefulset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let sts = apps_v1::StatefulSet::decode(data).ok()?;
    let mut obj = gen_statefulset_to_json(sts);
    obj["apiVersion"] = "apps/v1".into();
    obj["kind"] = "StatefulSet".into();
    Some(obj)
}
...
// `gen_deployment_spec_to_json`/`gen_deployment_status_to_json`/`gen_deployment_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.api.apps.v1.DeploymentSpec`/
// `DeploymentStatus`/`Deployment` descriptors — `metadata` delegates to `gen_object_meta_to_json`,
// `selector`/`template` delegate wholesale to `gen_apps_spec_to_json`, `strategy` delegates to a
// hand-written block (its `rollingUpdate.maxUnavailable`/`.maxSurge` are `IntOrString`, opaque to
// the mechanical walker), `paused` only ever emits `true`, and `conditions` delegates to the
// `apps_condition_to_json!` macro plus `lastUpdateTime`; see
// `build/codegen.rs::deployment_spec_delegated_field`/`deployment_status_delegated_field`/
// `deployment_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/deployment_spec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/deployment_status_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/deployment_gen.rs"));

pub fn decode_deployment_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let deploy = apps_v1::Deployment::decode(data).ok()?;
    let mut obj = gen_deployment_to_json(deploy);
    obj["apiVersion"] = "apps/v1".into();
    obj["kind"] = "Deployment".into();
    Some(obj)
}

// ---- Decoder A: DaemonSet --------------------------------------------------

// `gen_daemonset_spec_to_json`/`gen_daemonset_status_to_json`/`gen_daemonset_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.api.apps.v1.DaemonSetSpec`/`DaemonSetStatus`/
// `DaemonSet` descriptors — same delegation shape as Deployment/StatefulSet above, minus a
// `replicas` field (DaemonSet has none); see
// `build/codegen.rs::daemonset_spec_delegated_field`/`daemonset_status_delegated_field`/
// `daemonset_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/daemonset_spec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/daemonset_status_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/daemonset_gen.rs"));

pub fn decode_daemonset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let ds = apps_v1::DaemonSet::decode(data).ok()?;
    let mut obj = gen_daemonset_to_json(ds);
    obj["apiVersion"] = "apps/v1".into();
    obj["kind"] = "DaemonSet".into();
    Some(obj)
}

// ---- Decoder A: ReplicaSet -------------------------------------------------

// `gen_replicaset_spec_to_json`/`gen_replicaset_status_to_json`/`gen_replicaset_to_json` are
// generated by `build/codegen.rs` from the `.k8s.io.api.apps.v1.ReplicaSetSpec`/
// `ReplicaSetStatus`/`ReplicaSet` descriptors — same delegation shape as the other three apps/v1
// workload kinds above; see `build/codegen.rs::replicaset_spec_delegated_field`/
// `replicaset_status_delegated_field`/`replicaset_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/replicaset_spec_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/replicaset_status_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/replicaset_gen.rs"));

pub fn decode_replicaset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let rs = apps_v1::ReplicaSet::decode(data).ok()?;
    let mut obj = gen_replicaset_to_json(rs);
    obj["apiVersion"] = "apps/v1".into();
    obj["kind"] = "ReplicaSet".into();
    Some(obj)
}

// ---- Decoder A: ControllerRevision -----------------------------------------

// `gen_controllerrevision_to_json` is generated by `build/codegen.rs` from the
// `.k8s.io.api.apps.v1.ControllerRevision` descriptor — `metadata` delegates to
// `gen_object_meta_to_json`, `revision` unconditionally defaults to `0`, and `data` (a
// `RawExtension`) inlines the embedded document as JSON, silently omitted if the raw bytes don't
// parse; see `build/codegen.rs::controllerrevision_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/controllerrevision_gen.rs"));

pub fn decode_controllerrevision_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let cr = apps_v1::ControllerRevision::decode(data).ok()?;
    let mut obj = gen_controllerrevision_to_json(cr);
    obj["apiVersion"] = "apps/v1".into();
    obj["kind"] = "ControllerRevision".into();
    Some(obj)
}
...
mod tests {
    ...
    // ---- Byte-identical audit: codegen migration (apps/v1) ----
    //
    // Each test below reconstructs the pre-migration hand-written decode_*_proto_gen function
    // verbatim (they called the exact same gen_object_meta_to_json/gen_apps_spec_to_json/
    // gen_apps_int_or_string_to_json/apps_condition_to_json! helpers this migration left
    // untouched) and diffs its output against the codegen-generated gen_*_to_json this migration
    // installed, using Sentinel::sentinel() to exercise every field. A wrong field name, a
    // missing zero-filter guard, or a dropped IntOrString/RawExtension delegate in
    // build/codegen.rs would show up here as a JSON mismatch — a workload controller silently
    // misreading a Deployment/StatefulSet/DaemonSet/ReplicaSet/ControllerRevision's spec or
    // status is exactly the failure mode a byte-for-byte diff against the known-good
    // pre-migration output catches.

    #[test]
    fn codegen_migration_deployment_matches_pre_migration_hand_written_output() {
        fn old_decode_deployment_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            ...
            if let Some(spec) = obj.spec {
                ...
                let mut spec_json = gen_apps_spec_to_json(spec.selector, spec.template)
        ...
        let old = old_decode_deployment_proto_gen(&buf).expect("old path must decode");
        let new = decode_deployment_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old, new,
            "codegen-generated decode_deployment_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent \
             rollout-status or spec field drop (e.g. spec.paused, \
             spec.strategy.rollingUpdate.maxSurge, status.conditions[].lastUpdateTime) \
             introduced by this migration"
        );
    }

    #[test]
    fn codegen_migration_statefulset_matches_pre_migration_hand_written_output() {
        fn old_decode_statefulset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
        ...
        let old = old_decode_statefulset_proto_gen(&buf).expect("old path must decode");
        let new = decode_statefulset_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old, new,
            "codegen-generated decode_statefulset_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent drop of \
             volumeClaimTemplates/ordinals/persistentVolumeClaimRetentionPolicy or a status \
             revision field, breaking StatefulSet's own stable-storage/identity guarantees"
        );
    }

    #[test]
    fn codegen_migration_daemonset_matches_pre_migration_hand_written_output() {
        fn old_decode_daemonset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
        ...
        let old = old_decode_daemonset_proto_gen(&buf).expect("old path must decode");
        let new = decode_daemonset_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old, new,
            "codegen-generated decode_daemonset_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent drop of \
             updateStrategy/minReadySeconds/revisionHistoryLimit or a status scheduling gauge"
        );
    }

    #[test]
    fn codegen_migration_replicaset_matches_pre_migration_hand_written_output() {
        fn old_decode_replicaset_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
        ...
        let old = old_decode_replicaset_proto_gen(&buf).expect("old path must decode");
        let new = decode_replicaset_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old, new,
            "codegen-generated decode_replicaset_proto_gen must match the pre-migration \
             hand-written decoder field-for-field"
        );
    }

    #[test]
    fn codegen_migration_controllerrevision_matches_pre_migration_hand_written_output() {
        fn old_decode_controllerrevision_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
        ...
        // Not ControllerRevision::sentinel() wholesale: RawExtension::sentinel() would fill
        // `raw` with an arbitrary non-empty byte sequence that isn't valid JSON, which both the
        // old and new decoders silently (and correctly) drop if it fails to parse — a sentinel
        // value must be valid JSON here to actually exercise the "data survives" path.
        let cr = apps_v1::ControllerRevision {
            metadata: Some(meta_v1::ObjectMeta::sentinel()),
            data: Some(
                u7s_proto_generated::k8s::io::apimachinery::pkg::runtime::RawExtension {
        ...
        let old = old_decode_controllerrevision_proto_gen(&buf).expect("old path must decode");
        let new = decode_controllerrevision_proto_gen(&buf).expect("new path must decode");
        assert_eq!(
            old, new,
            "codegen-generated decode_controllerrevision_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent rollback \
             field drop"
        );
    }
}
