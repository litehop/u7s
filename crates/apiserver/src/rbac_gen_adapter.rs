...
// `gen_policy_rule_to_json`/`gen_subject_to_json`/`gen_role_ref_to_json` are generated by
// `build/codegen.rs` from the `.k8s.io.api.rbac.v1.PolicyRule`/`Subject`/`RoleRef` descriptors —
// `Subject.apiGroup` is the one field needing a delegate (it's inserted whenever `Option` is
// `Some`, even `Some("")`, unlike every other string field here); see
// `build/codegen.rs::subject_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/policy_rule_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/subject_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/role_ref_gen.rs"));
...
// `gen_clusterrole_to_json`/`gen_clusterrolebinding_to_json`/`gen_role_to_json`/
// `gen_rolebinding_to_json` are generated by `build/codegen.rs` from the
// `.k8s.io.api.rbac.v1.ClusterRole`/`ClusterRoleBinding`/`Role`/`RoleBinding` descriptors —
// `metadata` delegates to `gen_object_meta_to_json`, `rules`/`subjects` are emitted
// unconditionally (matching upstream's non-`omitempty` JSON tags), `roleRef` always decodes to at
// least `{}`, and `aggregationRule` reuses the hand-written `gen_label_selector_to_json`; see
// `build/codegen.rs::clusterrole_delegated_field`/`clusterrolebinding_delegated_field`/
// `role_delegated_field`/`rolebinding_delegated_field`.
include!(concat!(env!("OUT_DIR"), "/clusterrole_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/clusterrolebinding_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/role_gen.rs"));
include!(concat!(env!("OUT_DIR"), "/rolebinding_gen.rs"));

pub fn decode_clusterrole_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let cr = rbac_v1::ClusterRole::decode(data).ok()?;
    let mut obj = gen_clusterrole_to_json(cr);
    obj["apiVersion"] = "rbac.authorization.k8s.io/v1".into();
    obj["kind"] = "ClusterRole".into();
    Some(obj)
}
...
pub fn decode_clusterrolebinding_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut obj = gen_clusterrolebinding_to_json(crb);
    obj["apiVersion"] = "rbac.authorization.k8s.io/v1".into();
    obj["kind"] = "ClusterRoleBinding".into();
    Some(obj)
}

pub fn decode_role_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    let role = rbac_v1::Role::decode(data).ok()?;
    let mut obj = gen_role_to_json(role);
    obj["apiVersion"] = "rbac.authorization.k8s.io/v1".into();
    obj["kind"] = "Role".into();
    Some(obj)
}
...
pub fn decode_rolebinding_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
    ...
    let mut obj = gen_rolebinding_to_json(rb);
    obj["apiVersion"] = "rbac.authorization.k8s.io/v1".into();
    obj["kind"] = "RoleBinding".into();
    Some(obj)
}
...
mod tests {
    ...
    // ---- Byte-identical audit: codegen migration (Phase 4.2) ----
    //
    // Reconstructs the pre-migration hand-written `gen_policy_rule_to_json`/`gen_subject_to_json`/
    // `gen_role_ref_to_json`/`decode_clusterrole_proto_gen`/`decode_clusterrolebinding_proto_gen`/
    // `decode_role_proto_gen`/`decode_rolebinding_proto_gen` verbatim (as local `fn`s, since the
    // real names now resolve to the generated versions) and diffs their output against the
    // codegen-generated path this migration installed. A wrong JSON key, a missing `!= empty`
    // guard, or `Subject.apiGroup` losing its no-filter quirk would show up here as a JSON
    // mismatch — silently dropping a subject or a policy rule from an RBAC object is exactly the
    // failure mode a byte-for-byte diff against the known-good pre-migration output catches.
    #[test]
    fn codegen_migration_rbac_family_matches_pre_migration_hand_written_output() {
        fn old_gen_policy_rule_to_json(rule: rbac_v1::PolicyRule) -> serde_json::Value {
            let mut m = serde_json::Map::new();
        ...
        fn old_gen_subject_to_json(s: rbac_v1::Subject) -> serde_json::Value {
            let mut m = serde_json::Map::new();
        ...
        fn old_gen_role_ref_to_json(rr: rbac_v1::RoleRef) -> serde_json::Value {
            let mut m = serde_json::Map::new();
        ...
        fn old_decode_clusterrole_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let cr = rbac_v1::ClusterRole::decode(data).ok()?;
            let meta = gen_object_meta_to_json(cr.metadata.unwrap_or_default());
            let rules: Vec<serde_json::Value> = cr
                .rules
                .into_iter()
                .map(old_gen_policy_rule_to_json)
                .collect();
            ...
            if let Some(ar) = cr.aggregation_rule {
                if !ar.cluster_role_selectors.is_empty() {
                    let selectors: Vec<serde_json::Value> = ar
                        ...
                        .collect();
                    obj["aggregationRule"] =
                        serde_json::json!({ "clusterRoleSelectors": selectors });
                }
            }
            Some(obj)
        }

        fn old_decode_clusterrolebinding_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let crb = rbac_v1::ClusterRoleBinding::decode(data).ok()?;
            let meta = gen_object_meta_to_json(crb.metadata.unwrap_or_default());
            let subjects: Vec<serde_json::Value> = crb
                .subjects
                .into_iter()
                .map(old_gen_subject_to_json)
                .collect();
            let role_ref = crb
                .role_ref
                .map(old_gen_role_ref_to_json)
        ...
        fn old_decode_role_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let role = rbac_v1::Role::decode(data).ok()?;
        ...
        fn old_decode_rolebinding_proto_gen(data: &[u8]) -> Option<serde_json::Value> {
            let rb = rbac_v1::RoleBinding::decode(data).ok()?;
            let meta = gen_object_meta_to_json(rb.metadata.unwrap_or_default());
            let subjects: Vec<serde_json::Value> = rb
                .subjects
                .into_iter()
                .map(old_gen_subject_to_json)
                .collect();
        ...
        // (Sentinel's own contract) — this exercises `Subject.apiGroup`'s no-filter quirk,
        // `aggregationRule`'s selector list, and every PolicyRule verb/resource list, not just
        // the plain-string fields a less thorough fixture would happen to cover.
        let cr = rbac_v1::ClusterRole {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-clusterrole".to_string()),
                ..Default::default()
            }),
            rules: vec![rbac_v1::PolicyRule::sentinel()],
            aggregation_rule: Some(rbac_v1::AggregationRule::sentinel()),
        ...
        cr.encode(&mut cr_bytes).unwrap();
        assert_eq!(
            old_decode_clusterrole_proto_gen(&cr_bytes),
            decode_clusterrole_proto_gen(&cr_bytes),
            "codegen-generated decode_clusterrole_proto_gen must match the pre-migration \
             hand-written decoder field-for-field — any divergence here is a silent \
             authorization-observable field drop introduced by the Phase 4.2 migration"
        );

        // A ClusterRole with a present-but-empty AggregationRule (no selectors) must omit
        // `aggregationRule` entirely, not emit `{}` — the pre-migration decoder's own
        // `if !ar.cluster_role_selectors.is_empty()` guard, which
        // `clusterrole_delegated_field`'s own `aggregationRule` entry must replicate.
        let empty_aggregation_cr = rbac_v1::ClusterRole {
            metadata: Some(meta_v1::ObjectMeta::default()),
            rules: vec![],
            aggregation_rule: Some(rbac_v1::AggregationRule::default()),
        };
        let mut empty_aggregation_bytes = Vec::new();
        empty_aggregation_cr
            .encode(&mut empty_aggregation_bytes)
            .unwrap();
        assert_eq!(
            old_decode_clusterrole_proto_gen(&empty_aggregation_bytes),
            decode_clusterrole_proto_gen(&empty_aggregation_bytes),
            "a ClusterRole with a present-but-empty AggregationRule must still omit \
             `aggregationRule` entirely, matching the pre-migration decoder exactly"
        );

        let crb = rbac_v1::ClusterRoleBinding {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-crb".to_string()),
                ..Default::default()
            }),
            subjects: vec![rbac_v1::Subject::sentinel()],
            role_ref: Some(rbac_v1::RoleRef::sentinel()),
        };
        let mut crb_bytes = Vec::new();
        crb.encode(&mut crb_bytes).unwrap();
        assert_eq!(
            old_decode_clusterrolebinding_proto_gen(&crb_bytes),
            decode_clusterrolebinding_proto_gen(&crb_bytes),
            "codegen-generated decode_clusterrolebinding_proto_gen must match the pre-migration \
             hand-written decoder field-for-field"
        );

        let role = rbac_v1::Role {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-role".to_string()),
        ...
        role.encode(&mut role_bytes).unwrap();
        assert_eq!(
            old_decode_role_proto_gen(&role_bytes),
            decode_role_proto_gen(&role_bytes),
            "codegen-generated decode_role_proto_gen must match the pre-migration hand-written \
             decoder field-for-field"
        );

        let rb = rbac_v1::RoleBinding {
            metadata: Some(meta_v1::ObjectMeta {
                name: Some("audit-rb".to_string()),
                namespace: Some("kube-system".to_string()),
                ..Default::default()
            }),
            subjects: vec![rbac_v1::Subject::sentinel()],
        ...
        rb.encode(&mut rb_bytes).unwrap();
        assert_eq!(
            old_decode_rolebinding_proto_gen(&rb_bytes),
            decode_rolebinding_proto_gen(&rb_bytes),
            "codegen-generated decode_rolebinding_proto_gen must match the pre-migration \
             hand-written decoder field-for-field"
        );
    }
}
