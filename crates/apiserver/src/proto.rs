...
//
// These structs are reachable only from `#[cfg(test)]` code. Real production
// decoding for these Kubernetes resources goes through the protoc-compiled
// *_gen.rs types and their *_gen_adapter.rs decode_*_proto_gen() functions
// (e.g. core_gen_adapter::decode_serviceaccount_proto_gen,
// decode_persistentvolumeclaim_proto_gen). These hand-rolled structs exist
// only so tests can hand-assemble wire-format bytes without depending on the
// protoc-generated types. `#[derive(Message)]` self-constructs each type
// inside its Default/Message impls, so rustc's dead_code lint cannot see
// that nothing else ever builds one — gating them documents the truth
// explicitly (see PersistentVolumeClaim* below for the original precedent).
// ---------------------------------------------------------------------------
...
/// Used inside ServiceAccount.imagePullSecrets (embedded, not a separate JSON field).
#[cfg(test)]
#[derive(Clone, PartialEq, Message)]
