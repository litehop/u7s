...
/// `gen_pod_failure_policy_to_json` this migration retires, whose
/// `serde_json::json!({ "rules": rules })` has no `is_empty()` guard.
/// `PodFailurePolicyRule`/`PodFailurePolicyOnExitCodesRequirement`/
/// `PodFailurePolicyOnPodConditionsPattern` are inlined here rather than split into their own
/// `generate_*` functions: none of their own fields need a mechanical-walker override except
/// `onExitCodes` (inserted unconditionally once `Some`, unlike the mechanical walker's own
/// empty-object-omits default for a nested message), and none of these three types are reused
/// anywhere else in `batch/v1` — a shared top-level function would only add indirection.
fn pod_failure_policy_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "rules" => Some(
            "    let rules: Vec<serde_json::Value> = pfp.rules.into_iter().map(|r| {\n        let mut rule = serde_json::Map::new();\n        if let Some(v) = r.action.filter(|s| !s.is_empty()) {\n            rule.insert(\"action\".to_string(), serde_json::Value::String(v));\n        }\n        if let Some(ec) = r.on_exit_codes {\n            let mut ec_json = serde_json::Map::new();\n            if let Some(v) = ec.container_name.filter(|s| !s.is_empty()) {\n                ec_json.insert(\"containerName\".to_string(), serde_json::Value::String(v));\n            }\n            if let Some(v) = ec.operator.filter(|s| !s.is_empty()) {\n                ec_json.insert(\"operator\".to_string(), serde_json::Value::String(v));\n            }\n            if !ec.values.is_empty() {\n                ec_json.insert(\"values\".to_string(), ec.values.into_iter().map(serde_json::Value::from).collect::<Vec<_>>().into());\n            }\n            rule.insert(\"onExitCodes\".to_string(), serde_json::Value::Object(ec_json));\n        }\n        if !r.on_pod_conditions.is_empty() {\n            rule.insert(\"onPodConditions\".to_string(), r.on_pod_conditions.into_iter().map(|c| {\n                let mut cond = serde_json::Map::new();\n                if let Some(v) = c.r#type.filter(|s| !s.is_empty()) {\n                    cond.insert(\"type\".to_string(), serde_json::Value::String(v));\n                }\n                if let Some(v) = c.status.filter(|s| !s.is_empty()) {\n                    cond.insert(\"status\".to_string(), serde_json::Value::String(v));\n                }\n                serde_json::Value::Object(cond)\n            }).collect::<Vec<_>>().into());\n        }\n        serde_json::Value::Object(rule)\n    }).collect();\n    m.insert(\"rules\".to_string(), serde_json::Value::Array(rules));\n",
        ),
        _ => None,
    }
}

/// Generates `gen_pod_failure_policy_to_json`, replacing the hand-rolled function of the same
/// name.
pub fn generate_pod_failure_policy(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, POD_FAILURE_POLICY);
    let encode_stmts = generate_message_encode_only(
        &set,
        POD_FAILURE_POLICY,
        message,
        pod_failure_policy_delegated_field,
        "pfp",
        "m",
    );
...
const SUCCESS_POLICY: &str = ".k8s.io.api.batch.v1.SuccessPolicy";

/// `rules` must be emitted unconditionally, even as `[]`, matching the pre-migration
/// `gen_success_policy_to_json` this migration retires. `SuccessPolicyRule`'s own fields
/// (`succeededIndexes`/`succeededCount`) need no override — both already match the mechanical
/// walker's defaults — but the whole array is inlined here rather than split into its own
/// `generate_success_policy_rule` function, for the same not-reused-elsewhere reason
/// `pod_failure_policy_delegated_field` documents.
fn success_policy_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "rules" => Some(
            "    let rules: Vec<serde_json::Value> = sp.rules.into_iter().map(|r| {\n        let mut rule = serde_json::Map::new();\n        if let Some(v) = r.succeeded_indexes.filter(|s| !s.is_empty()) {\n            rule.insert(\"succeededIndexes\".to_string(), serde_json::Value::String(v));\n        }\n        if let Some(v) = r.succeeded_count {\n            rule.insert(\"succeededCount\".to_string(), serde_json::Value::Number(v.into()));\n        }\n        serde_json::Value::Object(rule)\n    }).collect();\n    m.insert(\"rules\".to_string(), serde_json::Value::Array(rules));\n",
        ),
        _ => None,
    }
}

/// Generates `gen_success_policy_to_json`, replacing the hand-rolled function of the same name.
pub fn generate_success_policy(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, SUCCESS_POLICY);
    let encode_stmts = generate_message_encode_only(
        &set,
        SUCCESS_POLICY,
        message,
        success_policy_delegated_field,
        "sp",
        "m",
    );
...
const JOB_SPEC: &str = ".k8s.io.api.batch.v1.JobSpec";

/// `suspend`/`manualSelector` are plain, true-only-emitting bools — the same class
/// `container_delegated_field`'s `stdin`/`stdinOnce`/`tty` document — matching the pre-migration
/// `gen_job_spec_to_json` this migration retires exactly. `selector` delegates to the existing
/// hand-written `gen_label_selector_to_json`, inserted unconditionally once `Some` (even an empty
/// `LabelSelector`), unlike the mechanical walker's own empty-object-omits default for a nested
/// message. `podFailurePolicy`/`successPolicy` delegate to the separately generated
/// `gen_pod_failure_policy_to_json`/`gen_success_policy_to_json`, also inserted unconditionally
/// once `Some`. `template` is always present, defaulting to `{}` whenever `spec.template` is
/// absent or maps to an empty object — mirrors the pre-migration decoder's own explicit default
/// rather than omitting the key.
fn job_spec_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "suspend" => Some(
            "    if let Some(true) = spec.suspend {\n        m.insert(\"suspend\".to_string(), serde_json::Value::Bool(true));\n    }\n",
        ),
        "manualSelector" => Some(
            "    if let Some(true) = spec.manual_selector {\n        m.insert(\"manualSelector\".to_string(), serde_json::Value::Bool(true));\n    }\n",
        ),
        "selector" => Some(
            "    if let Some(sel) = spec.selector {\n        m.insert(\"selector\".to_string(), gen_label_selector_to_json(sel));\n    }\n",
        ),
        "podFailurePolicy" => Some(
            "    if let Some(pfp) = spec.pod_failure_policy {\n        m.insert(\"podFailurePolicy\".to_string(), gen_pod_failure_policy_to_json(pfp));\n    }\n",
        ),
        "successPolicy" => Some(
...
/// Generates `gen_job_spec_to_json`, replacing the hand-rolled function of the same name.
pub fn generate_job_spec(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, JOB_SPEC);
    let encode_stmts = generate_message_encode_only(
        &set,
        JOB_SPEC,
        message,
        job_spec_delegated_field,
        "spec",
        "m",
    );
    ...
    out.push_str("// @generated by crates/apiserver/build/codegen.rs — do not hand-edit.\n");
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str("fn gen_job_spec_to_json(spec: batch_v1::JobSpec) -> serde_json::Value {\n");
    out.push_str("    let mut m = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(m)\n");
    out.push_str("}\n");
...
const JOB_STATUS: &str = ".k8s.io.api.batch.v1.JobStatus";

/// `conditions` needs `type`/`status` to always emit (even `""`, via `unwrap_or_default`) rather
/// than the mechanical walker's own filter-if-non-empty default for strings — Job controllers do
/// exact string comparisons against these two fields. `lastProbeTime`/`lastTransitionTime`
/// (`.meta.v1.Time`, RFC3339) need the same delegate every `Time`-typed field in this codebase
/// needs: the mechanical walker has no `OPAQUE_MESSAGES`-style scalar-conversion rule and would
/// otherwise recurse into `Time`'s own `seconds`/`nanos` fields as a generic nested object.
/// `startTime`/`completionTime` are the same `Time` delegate one level up. `uncountedTerminatedPods`
/// must be inserted unconditionally once `Some`, even if both its `succeeded`/`failed` lists are
/// empty — upstream's Job controller (`JobTrackingWithFinalizers`) nil-derefs on an absent field
/// once it starts tracking a Job (`job_controller.go:1568`), the same reason
/// `sentinel_completeness_decode_job_proto_gen`'s own present-but-empty regression test exists.
fn job_status_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "conditions" => Some(
        ...
        "startTime" => Some(
            "    if let Some(t) = status.start_time.as_ref() {\n        if let Some(secs) = t.seconds.filter(|&s| s > 0) {\n            m.insert(\"startTime\".to_string(), crate::util::secs_to_rfc3339(secs).into());\n        }\n    }\n",
        ),
        "completionTime" => Some(
            "    if let Some(t) = status.completion_time.as_ref() {\n        if let Some(secs) = t.seconds.filter(|&s| s > 0) {\n            m.insert(\"completionTime\".to_string(), crate::util::secs_to_rfc3339(secs).into());\n        }\n    }\n",
        ),
        "uncountedTerminatedPods" => Some(
            "    if let Some(utp) = status.uncounted_terminated_pods {\n        let mut utp_map = serde_json::Map::new();\n        if !utp.succeeded.is_empty() {\n            utp_map.insert(\"succeeded\".to_string(), utp.succeeded.into_iter().map(serde_json::Value::String).collect::<Vec<_>>().into());\n        }\n        if !utp.failed.is_empty() {\n            utp_map.insert(\"failed\".to_string(), utp.failed.into_iter().map(serde_json::Value::String).collect::<Vec<_>>().into());\n        }\n        m.insert(\"uncountedTerminatedPods\".to_string(), serde_json::Value::Object(utp_map));\n    }\n",
        ),
        _ => None,
    }
}

/// Generates `gen_job_status_to_json`, replacing the hand-rolled `status` assembly block of the
/// pre-migration `decode_job_proto_gen` this migration retires.
pub fn generate_job_status(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, JOB_STATUS);
    let encode_stmts = generate_message_encode_only(
        &set,
        JOB_STATUS,
        message,
        job_status_delegated_field,
        "status",
        "m",
    );
    ...
    out.push_str("// @generated by crates/apiserver/build/codegen.rs — do not hand-edit.\n");
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str("fn gen_job_status_to_json(status: batch_v1::JobStatus) -> serde_json::Value {\n");
    out.push_str("    let mut m = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(m)\n");
    out.push_str("}\n");
...
const JOB: &str = ".k8s.io.api.batch.v1.Job";

/// `metadata` delegates for the same reason as every other Kind in this file. `spec` is inserted
/// unconditionally once `Some` (matching the pre-migration `decode_job_proto_gen` this migration
/// retires, whose `out["spec"] = gen_job_spec_to_json(spec);` has no emptiness guard). `status`
/// only inserts once the resulting object is non-empty, matching that same decoder's own
/// `if status_json...!m.is_empty()` guard.
fn job_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "metadata" => Some(
            "    obj.insert(\"metadata\".to_string(), gen_object_meta_to_json(job.metadata.unwrap_or_default()));\n",
        ),
        "spec" => Some(
            "    if let Some(spec) = job.spec {\n        obj.insert(\"spec\".to_string(), gen_job_spec_to_json(spec));\n    }\n",
        ),
        "status" => Some(
            "    if let Some(status) = job.status {\n        let status_json = gen_job_status_to_json(status);\n        if status_json.as_object().is_some_and(|m| !m.is_empty()) {\n            obj.insert(\"status\".to_string(), status_json);\n        }\n    }\n",
        ),
        _ => None,
    }
}

/// Generates `gen_job_to_json`, replacing the message-walking body of the hand-rolled
/// `decode_job_proto_gen` this migration retires (the entry point itself stays hand-written — see
/// `generate_namespace`'s doc for why; `Job` has no `encode_job_proto_gen` today, so this is
/// decode-only in the same sense).
pub fn generate_job(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, JOB);
    let encode_stmts =
        generate_message_encode_only(&set, JOB, message, job_delegated_field, "job", "obj");

    let mut out = String::new();
    out.push_str("// @generated by crates/apiserver/build/codegen.rs — do not hand-edit.\n");
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str("fn gen_job_to_json(job: batch_v1::Job) -> serde_json::Value {\n");
    out.push_str("    let mut obj = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(obj)\n");
    out.push_str("}\n");
...
const CRON_JOB_SPEC: &str = ".k8s.io.api.batch.v1.CronJobSpec";

/// `suspend` is the same true-only bool class as `job_spec_delegated_field`'s own entry.
/// `jobTemplate` needs a fully custom delegate: `metadata` defaults to
/// `{"creationTimestamp": null}` and `spec` defaults to `{"template": {}}` whenever
/// `spec.job_template`/its own `metadata`/`spec` are absent — matching the pre-migration
/// `decode_cronjob_proto_gen` this migration retires exactly, and reusing the separately
/// generated `gen_job_spec_to_json` for the nested `JobSpec`.
fn cronjob_spec_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "suspend" => Some(
            "    if let Some(true) = spec.suspend {\n        m.insert(\"suspend\".to_string(), serde_json::Value::Bool(true));\n    }\n",
        ),
        "jobTemplate" => Some(
            "    let jt_meta = spec.job_template.as_ref().and_then(|jt| jt.metadata.clone()).map(gen_object_meta_to_json).unwrap_or_else(|| serde_json::json!({\"creationTimestamp\": serde_json::Value::Null}));\n    let jt_spec = spec.job_template.and_then(|jt| jt.spec).map(gen_job_spec_to_json).unwrap_or_else(|| serde_json::json!({\"template\": {}}));\n    m.insert(\"jobTemplate\".to_string(), serde_json::json!({ \"metadata\": jt_meta, \"spec\": jt_spec }));\n",
        ),
        _ => None,
    }
}

/// Generates `gen_cronjob_spec_to_json`, replacing the hand-rolled `spec` assembly block of the
/// pre-migration `decode_cronjob_proto_gen` this migration retires.
pub fn generate_cronjob_spec(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, CRON_JOB_SPEC);
    let encode_stmts = generate_message_encode_only(
        &set,
        CRON_JOB_SPEC,
        message,
        cronjob_spec_delegated_field,
        "spec",
        "m",
    );
    ...
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str(
        "fn gen_cronjob_spec_to_json(spec: batch_v1::CronJobSpec) -> serde_json::Value {\n",
    );
    out.push_str("    let mut m = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(m)\n");
...
const CRON_JOB_STATUS: &str = ".k8s.io.api.batch.v1.CronJobStatus";

/// `active` needs a fully custom delegate: each `.k8s.io.api.core.v1.ObjectReference` entry is
/// dropped entirely when both `name`/`namespace` are empty, matching the pre-migration
/// `decode_cronjob_proto_gen` this migration retires exactly rather than reusing
/// `core_gen_adapter.rs`'s shared `gen_object_reference_to_json` (a private fn in that module,
/// and one with no empty-entry filter). `lastScheduleTime`/`lastSuccessfulTime` are the same
/// `Time` delegate every `Time`-typed field in this codebase needs.
fn cronjob_status_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "active" => Some(
            "    if !status.active.is_empty() {\n        m.insert(\"active\".to_string(), status.active.iter().filter_map(|r| {\n            let name = r.name.as_deref().unwrap_or(\"\");\n            let ns = r.namespace.as_deref().unwrap_or(\"\");\n            if name.is_empty() && ns.is_empty() {\n                return None;\n            }\n            let mut entry = serde_json::json!({});\n            if !name.is_empty() {\n                entry[\"name\"] = name.to_string().into();\n            }\n            if !ns.is_empty() {\n                entry[\"namespace\"] = ns.to_string().into();\n            }\n            if let Some(v) = r.kind.as_deref().filter(|s| !s.is_empty()) {\n                entry[\"kind\"] = v.to_string().into();\n            }\n            if let Some(v) = r.api_version.as_deref().filter(|s| !s.is_empty()) {\n                entry[\"apiVersion\"] = v.to_string().into();\n            }\n            if let Some(v) = r.uid.as_deref().filter(|s| !s.is_empty()) {\n                entry[\"uid\"] = v.to_string().into();\n            }\n            if let Some(v) = r.resource_version.as_deref().filter(|s| !s.is_empty()) {\n                entry[\"resourceVersion\"] = v.to_string().into();\n            }\n            if let Some(v) = r.field_path.as_deref().filter(|s| !s.is_empty()) {\n                entry[\"fieldPath\"] = v.to_string().into();\n            }\n            Some(entry)\n        }).collect::<Vec<_>>().into());\n    }\n",
        ),
        "lastScheduleTime" => Some(
            "    if let Some(t) = status.last_schedule_time.as_ref() {\n        if let Some(secs) = t.seconds.filter(|&s| s > 0) {\n            m.insert(\"lastScheduleTime\".to_string(), crate::util::secs_to_rfc3339(secs).into());\n        }\n    }\n",
        ),
        "lastSuccessfulTime" => Some(
            "    if let Some(t) = status.last_successful_time.as_ref() {\n        if let Some(secs) = t.seconds.filter(|&s| s > 0) {\n            m.insert(\"lastSuccessfulTime\".to_string(), crate::util::secs_to_rfc3339(secs).into());\n        }\n    }\n",
        ),
        _ => None,
    }
}

/// Generates `gen_cronjob_status_to_json`, replacing the hand-rolled `status` assembly block of
/// the pre-migration `decode_cronjob_proto_gen` this migration retires.
pub fn generate_cronjob_status(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, CRON_JOB_STATUS);
    let encode_stmts = generate_message_encode_only(
        &set,
        CRON_JOB_STATUS,
        message,
        cronjob_status_delegated_field,
        "status",
        "m",
    );
    ...
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str(
        "fn gen_cronjob_status_to_json(status: batch_v1::CronJobStatus) -> serde_json::Value {\n",
    );
    out.push_str("    let mut m = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(m)\n");
...
const CRON_JOB: &str = ".k8s.io.api.batch.v1.CronJob";

/// `metadata` delegates for the same reason as every other Kind in this file. `spec`/`status`
/// mirror `job_delegated_field`'s own entries: `spec` inserts unconditionally once `Some`,
/// `status` only once the resulting object is non-empty — matching the pre-migration
/// `decode_cronjob_proto_gen` this migration retires exactly.
fn cronjob_delegated_field(field_name: &str) -> Option<&'static str> {
    match field_name {
        "metadata" => Some(
            "    obj.insert(\"metadata\".to_string(), gen_object_meta_to_json(cj.metadata.unwrap_or_default()));\n",
        ),
        "spec" => Some(
            "    if let Some(spec) = cj.spec {\n        obj.insert(\"spec\".to_string(), gen_cronjob_spec_to_json(spec));\n    }\n",
        ),
        "status" => Some(
            "    if let Some(status) = cj.status {\n        let status_json = gen_cronjob_status_to_json(status);\n        if status_json.as_object().is_some_and(|m| !m.is_empty()) {\n            obj.insert(\"status\".to_string(), status_json);\n        }\n    }\n",
        ),
        _ => None,
    }
}

/// Generates `gen_cronjob_to_json`, replacing the message-walking body of the hand-rolled
/// `decode_cronjob_proto_gen` this migration retires (the entry point itself stays hand-written —
/// see `generate_namespace`'s doc for why; `CronJob` has no `encode_cronjob_proto_gen` today, so
/// this is decode-only in the same sense).
pub fn generate_cronjob(descriptor_bytes: &[u8]) -> String {
    let set = FileDescriptorSet::decode(descriptor_bytes)
        .expect("descriptor set emitted by build.rs must decode");
    let message = find_message(&set, CRON_JOB);
    let encode_stmts = generate_message_encode_only(
        &set,
        CRON_JOB,
        message,
        cronjob_delegated_field,
        "cj",
        "obj",
    );
    ...
    out.push_str("// @generated by crates/apiserver/build/codegen.rs — do not hand-edit.\n");
    out.push_str("// Regenerated on every `cargo build -p u7s-apiserver`.\n\n");
    out.push_str("fn gen_cronjob_to_json(cj: batch_v1::CronJob) -> serde_json::Value {\n");
    out.push_str("    let mut obj = serde_json::Map::new();\n");
    out.push_str(&encode_stmts);
    out.push_str("    serde_json::Value::Object(obj)\n");
    out.push_str("}\n");
