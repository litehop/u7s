...
fn main() {
    ...
    // Emit the FileDescriptorSet alongside the generated structs so u7s-apiserver's sentinel
    // completeness tests (proto_descriptor.rs) and its build/codegen.rs can derive their
    // expected-JSON-key lists from the .proto schema itself instead of hand-maintaining them —
    // see this crate's `DESCRIPTOR_BYTES` export.
    ...
    // (see u7s-apiserver's core_gen_adapter.rs tests module) can build a fully-populated instance
    // of any message type without hand-listing its fields. Safe as a "." (root) pattern only
    // because the vendored .proto schema has no `oneof`/`enum` types for it to hit (verified by
    // grep); if one is ever added, u7s-sentinel-derive fails the build loudly instead of silently
